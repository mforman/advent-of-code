...
from itertools import filterfalse, permutations, product, repeat
import heapq
...
def find_path(initial, goal, max_steps):
    queue = []
...
def main():
    initial = (1, 1)
    max_steps = 50
    goals = [g for g in product(range(max_steps+1), repeat=2) if is_space(g) and find_path(initial, g, max_steps) >= 0]

    print('Found {} points in at most {} steps'.format(len(goals), max_steps))
